...
artifacts: docker-final
	...
	@CID=$$(docker create seeker.final) && \
	  docker cp $$CID:/output/seeker-$(PROJECT_VERSION)-x86_64-linux.tar.gz $(DOCKER_OUT_DIR)/ && \
	  docker cp $$CID:/output/seeker-$(PROJECT_VERSION)-x86_64-linux-cuda.tar.gz $(DOCKER_OUT_DIR)/ && \
	  docker cp $$CID:/output/seeker-$(PROJECT_VERSION)-aarch64-jetson.tar.gz $(DOCKER_OUT_DIR)/ && \
	  docker cp $$CID:/output/seeker-$(PROJECT_VERSION)-aarch64-rpi.tar.gz $(DOCKER_OUT_DIR)/ && \
	  docker cp $$CID:/output/seeker-$(PROJECT_VERSION)-riscv64-linux.tar.gz $(DOCKER_OUT_DIR)/ && \
	  docker rm $$CID
...
docker-prune:
	$(call log,docker,Removing seeker project images)
	@docker images --format '{{.Repository}}:{{.Tag}}' | grep '^seeker\.' | xargs -r docker rmi -f 2>/dev/null || true
	@docker image prune -f
...
docker-disk-usage:
	...
	@docker system df
	@echo ""
	@echo "Seeker Image Sizes:"
	@docker images --format "  {{.Repository}}:{{.Tag}} => {{.Size}}" | grep seeker | sort
