# ==============================================================================
# seeker/Cuda.cmake - CUDA source integration and NVML support
# ==============================================================================

include_guard(GLOBAL)

# ------------------------------------------------------------------------------
# seeker_cuda_sources(<target> FILES <cu...>
#                      [NO_CUDART] [SEPARABLE] [RESOLVE_DEVICE_SYMBOLS]
#                      [DEFINE <n>])
#
# Add CUDA source files to an existing target (no-op when CUDA inactive).
# Links CUDA::cudart by default. Defines SEEKER_HAS_CUDA=1 unless overridden.
# ------------------------------------------------------------------------------
function (seeker_cuda_sources _target)
  ...
  if (NOT CU_FILES)
    message(FATAL_ERROR "seeker_cuda_sources: no FILES specified for '${_target}'")
  endif ()
  ...
  if (NOT _cuda_define)
    set(_cuda_define "SEEKER_HAS_CUDA")
  endif ()
  ...
  if (NOT CU_NO_CUDART)
    if (TARGET CUDA::cudart)
      ...
      message(WARNING "seeker_cuda_sources: CUDA::cudart not found")
    endif ()
  endif ()
endfunction ()
...
function (_seeker_cuda_lib_paths _out_paths _out_stub_paths)
  set(_paths)
  set(_stubs)
...
function (_seeker_cuda_inc_paths _out_paths)
  set(_paths)
...
function (_seeker_cuda_target_scope _target _out_scope)
  get_target_property(_tgt_type "${_target}" TYPE)
  if (_tgt_type STREQUAL "INTERFACE_LIBRARY")
    set(${_out_scope}
        INTERFACE
...
# ------------------------------------------------------------------------------
function (seeker_nvml_enable _target)
  ...
  _seeker_cuda_target_scope(${_target} _scope)
  ...
  _seeker_cuda_inc_paths(_inc_paths)
  _seeker_cuda_lib_paths(_lib_paths _stub_paths)
...
# ------------------------------------------------------------------------------
function (seeker_cupti_enable _target)
  seeker_guard(${_target})
