# ==============================================================================
# seeker/Tooling.cmake - Documentation, compression, and static analysis
# ==============================================================================
...
# ------------------------------------------------------------------------------
function (seeker_add_doxygen _target)
  if (NOT PROJECT_BUILD_DOCS)
    return()
  endif ()

  seeker_guard(${_target})
...
# ------------------------------------------------------------------------------
# seeker_install_docs(<path> [DESTINATION <subdir>])
#
...
# ------------------------------------------------------------------------------
function (seeker_install_docs _path)
  if (CMAKE_BUILD_TYPE)
...
# ------------------------------------------------------------------------------
function (seeker_add_upx_copy _target)
  if (NOT ENABLE_UPX)
    return()
  endif ()
...
# Cache options
if (NOT DEFINED SEEKER_CLANG_TIDY_CUDA_STUBS)
  set(SEEKER_CLANG_TIDY_CUDA_STUBS
      ON
      CACHE BOOL "Generate stub headers for clang-tidy CUDA"
  )
endif ()
if (NOT DEFINED SEEKER_CLANG_TIDY_CUDA_STUB_HEADERS)
  set(SEEKER_CLANG_TIDY_CUDA_STUB_HEADERS
      "texture_fetch_functions.h"
      CACHE STRING "CUDA headers to stub (semicolon-separated)"
  )
endif ()
if (NOT DEFINED SEEKER_CLANG_TIDY_CUDA_MUTE_FRONTEND)
  set(SEEKER_CLANG_TIDY_CUDA_MUTE_FRONTEND
      ON
      CACHE BOOL "Silence frontend warnings during tidy"
  )
endif ()
if (NOT DEFINED SEEKER_CLANG_TIDY_CUDA_HEADER_FILTER)
  string(REPLACE "\\" "\\\\" _seeker_src_root "${PROJECT_SOURCE_DIR}")
  set(SEEKER_CLANG_TIDY_CUDA_HEADER_FILTER
      "^${_seeker_src_root}/src/"
      CACHE STRING "Header filter regex for clang-tidy"
  )
endif ()

# ------------------------------------------------------------------------------
# seeker_clang_tidy_cuda(<target> FILES <cu...> [ALLOW_FAILURE])
#
# Run clang-tidy on CUDA sources via Clang's CUDA frontend.
# No-op when CUDA not found, ENABLE_CLANG_TIDY=OFF, or no FILES.
# ------------------------------------------------------------------------------
function (seeker_clang_tidy_cuda _target)
  ...
  # SM architecture
  if (DEFINED SEEKER_CLANG_TIDY_CUDA_ARCH)
    set(_sm "${SEEKER_CLANG_TIDY_CUDA_ARCH}")
  ...
  # Stub headers
  if (SEEKER_CLANG_TIDY_CUDA_STUBS)
    set(_stub_dir "${CMAKE_BINARY_DIR}/clang_tidy_cuda_stubs")
    file(MAKE_DIRECTORY "${_stub_dir}")
    foreach (_hdr IN LISTS SEEKER_CLANG_TIDY_CUDA_STUB_HEADERS)
      if (NOT _hdr)
        continue()
      endif ()
...
"
  ...
  foreach (_src IN LISTS CT_FILES)
    ...
    list(APPEND _args "--header-filter='${SEEKER_CLANG_TIDY_CUDA_HEADER_FILTER}'")
    if (SEEKER_CLANG_TIDY_CUDA_MUTE_FRONTEND)
