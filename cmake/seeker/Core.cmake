# ==============================================================================
# seeker/Core.cmake - Foundation utilities for Seeker CMake infrastructure
# ==============================================================================
...
# ------------------------------------------------------------------------------
function (seeker_require)
  foreach (_var IN LISTS ARGN)
    if (NOT DEFINED ${_var} OR "${${_var}}" STREQUAL "")
      message(FATAL_ERROR "seeker_require: '${_var}' is required but not set")
    endif ()
  endforeach ()
endfunction ()
...
# ------------------------------------------------------------------------------
function (seeker_guard _target)
  if (NOT TARGET "${_target}")
    message(FATAL_ERROR "seeker_guard: target '${_target}' does not exist")
  endif ()
endfunction ()
...
# ------------------------------------------------------------------------------
# seeker_is_cuda_target(<target> <out_var>)
#
# Set <out_var> to TRUE if target has CUDA sources, FALSE otherwise.
# ------------------------------------------------------------------------------
function (seeker_is_cuda_target _target _out_var)
  set(_has_cuda FALSE)
...
# ------------------------------------------------------------------------------
# seeker_standard_optins(<target>)
#
# Apply standard opt-in features to a target:
#   - seeker_enable_coverage (Debug only, respects ENABLE_COVERAGE)
#   - seeker_add_doxygen (respects PROJECT_BUILD_DOCS)
#   - seeker_add_upx_copy (respects ENABLE_UPX)
#   - seeker_clang_tidy_cuda (if target has CUDA sources, respects ENABLE_CLANG_TIDY)
#
...
# ------------------------------------------------------------------------------
function (seeker_standard_optins _target)
  ...
  seeker_add_doxygen(${_target})

  # UPX compression
  seeker_add_upx_copy(${_target})

  # CUDA-specific: clang-tidy on .cu files
  seeker_is_cuda_target(${_target} _has_cuda)
  if (_has_cuda)
    get_target_property(_sources ${_target} SOURCES)
    set(_cu_files)
    foreach (_src IN LISTS _sources)
    ...
    endforeach ()
    if (_cu_files)
      seeker_clang_tidy_cuda(${_target} FILES ${_cu_files} ALLOW_FAILURE)
    endif ()
  endif ()
endfunction ()
