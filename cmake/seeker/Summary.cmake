# ==============================================================================
# seeker/Summary.cmake - Configure-time summary banner
# ==============================================================================
...
function (_seeker_dumpmachine _exe _out_triple)
  if (NOT _exe)
    set(${_out_triple}
        ""
        PARENT_SCOPE
...
function (_seeker_compiler_info _lang _out_prefix)
  if (_lang STREQUAL "C")
  ...
  if (_lang STREQUAL "CUDA")
    _seeker_dumpmachine("${CMAKE_CUDA_HOST_COMPILER}" _triple)
  else ()
    _seeker_dumpmachine("${_exe}" _triple)
  endif ()
...
function (_seeker_join _list_var _sep _out_str)
  set(_tmp "${${_list_var}}")
  if (NOT _tmp)
    set(${_out_str}
        ""
...
function (_seeker_yesno _val _out_str)
  if (_val)
    set(${_out_str}
        "YES"
        PARENT_SCOPE
...
function (_seeker_tick _cond _out_str)
  if (_cond)
    set(${_out_str}
        "[OK]"
        PARENT_SCOPE
...
function (_seeker_contains _str _sub _out_bool)
  if (_str STREQUAL "")
    set(${_out_bool}
        FALSE
        PARENT_SCOPE
...
function (_seeker_expected_triple _out_triple)
...
function (_seeker_cuda_target_libdir _out_libdir _out_root)
  set(_root "")
  set(_libdir "")
...
# ------------------------------------------------------------------------------
# seeker_print_config_summary()
#
# Emit configuration banner showing build settings, compilers, and CUDA status.
# ------------------------------------------------------------------------------
function (seeker_print_config_summary)
  # Build type
  if (CMAKE_CONFIGURATION_TYPES)
    _seeker_join(CMAKE_CONFIGURATION_TYPES ", " _cfgs)
  ...
  # Cross-compilation
  _seeker_yesno("${CMAKE_CROSSCOMPILING}" _is_cross)
  ...
  # Compilers
  _seeker_compiler_info("C" CINFO)
  _seeker_compiler_info("CXX" CXXINFO)
  if (CMAKE_CUDA_COMPILER)
    _seeker_compiler_info("CUDA" CUDAINFO)
  endif ()

  _seeker_expected_triple(_expected_triple)
  ...
  if (CMAKE_CROSSCOMPILING AND _expected_triple)
    ...
    _seeker_contains("${CINFO_TRIPLE}" "${_expected_triple}" _c_ok)
    _seeker_contains("${CXXINFO_TRIPLE}" "${_expected_triple}" _cxx_ok)
    if (CMAKE_CUDA_HOST_COMPILER)
      _seeker_dumpmachine("${CMAKE_CUDA_HOST_COMPILER}" _cuda_host_triple)
      _seeker_contains("${_cuda_host_triple}" "${_expected_triple}" _cuda_host_ok)
    endif ()
    _seeker_tick(${_c_ok} _c_tick)
    _seeker_tick(${_cxx_ok} _cxx_tick)
    _seeker_tick(${_cuda_host_ok} _cuda_host_tick)
  endif ()
  ...
  if (_cuda_enabled AND CUDAToolkit_FOUND)
    _seeker_cuda_target_libdir(_cuda_libdir _cuda_root)
    if (NOT _cuda_root)
  ...
  _seeker_tick(${_cudart_found} _cudart_tick)
  _seeker_tick(${_cudadevrt_found} _cudadevrt_tick)
  ...
  if (CMAKE_CROSSCOMPILING)
    ...
    if (_cuda_libdir AND _exe_link_flags)
      _seeker_contains("${_exe_link_flags}" "-Wl,-rpath-link,${_cuda_libdir}" _rpathlink_ok)
      _seeker_contains("${_exe_link_flags}" "-L${_cuda_libdir}" _l_ok)
    endif ()
    _seeker_tick(${_l_ok} _l_tick)
    _seeker_tick(${_rpathlink_ok} _rpath_tick)
  else ()
    set(_l_tick "n/a")
    set(_rpath_tick "n/a")
  endif ()
  ...
  # Feature toggles
  _seeker_yesno("${ENABLE_UPX}" _enable_upx)
  _seeker_yesno("${ENABLE_COVERAGE}" _enable_cov)
  _seeker_yesno("${ENABLE_CLANG_TIDY}" _enable_tidy)
  _seeker_yesno("${SEEKER_BUILD_GPU}" _build_gpu)
  _seeker_yesno("${SEEKER_BUILD_TOOLS}" _build_tools)
  ...
  _seeker_yesno("${CMAKE_BUILD_WITH_INSTALL_RPATH}" _use_install_rpath)
  ...
  # Banner output
  message(STATUS "-------------------------------------------------------------------------------")
  message(STATUS "Seeker - Configure Summary")
  message(STATUS "-------------------------------------------------------------------------------")
  message(
    STATUS
  ...
  if (_cuda_enabled)
    message(STATUS "  SEEKER_BUILD_GPU   : ${_build_gpu}")
  else ()
    message(STATUS "  SEEKER_BUILD_GPU   : ${_build_gpu} (no CUDA toolkit)")
  endif ()
  ...
  message(STATUS "  SEEKER_BUILD_TOOLS : ${_build_tools}")
